...
用 TypeScript 编写，单文件实现：输入「已有存款 / 存款方式 / 每期攒多少 / 年化 / 目标 / 开始日期」，逐年算出攒钱进度与达标时间，支持按年存与按月存。业务说明见 README.md，本文件记录架构与口径。
...
## 计算口径（用户确认，勿随意改）
- 存款方式二选一：**按年存**（`calculateYearly`）/ **按月存**（`calculateMonthly`），`calculateSavingPlan` 只做前置判断后分发
- **按年**：年初存入；年利息 = 年初余额（含当年存入）× 年化；年末 = ×(1+年化)；年内利息按月线性累积 `第 m 月余额 = 年初余额 × (1 + 年化 × m/12)`，m 从 0 起（m=0 即年初存入后立即达标）
- **按月**：月初存入；等效月利率 `r = (1+年化)^(1/12) − 1`（按月复利 12 次正好等于年化，比 `年化/12` 名义月利率更贴合真实年化）；每月 `余额 = (余额 + 月存) × (1+r)`，月末判断达标
...
- 汇总：总投入本金 = 初始存款 + 累计存入（按年=年存×年数，按月=月存×月数）；总利息 = 最终金额 − 总投入本金

## 输入约定
- **金额均以「万元」为单位输入**（如 `250` 表示 250 万），代码内 `×10000` 转回元；记忆与展示仍以元为准（读回默认值时 `/10000` 转万元）
- `存款方式` 用 `select`（按年/按月），金额项文案随方式变化（每年攒多少 / 每月攒多少），默认值按方式换算（按月=年额/12，按年=月额×12）
- 年化收益率**输入 `2` 表示 2%**（代码内 `/100` 转小数）
- 各数字项 `step: "any"`，非法值重新提示：存款/每期存入/年化 ≥0，目标 >0
...
- 结构：`{ depositFrequency, savingAmount, initialDeposit, annualRatePercent, target, startDate }`；`loadLast` 兼容旧结构（无 `depositFrequency`/`savingAmount` 时，把 `yearlySaving` 视为按年）
